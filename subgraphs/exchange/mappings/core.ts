...
import { updatePairDayData, updateTokenDayData, updateMoonwalkerDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let moonwalker = MoonwalkerFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  moonwalker.totalLiquidityBNB = moonwalker.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  moonwalker.totalLiquidityBNB = moonwalker.totalLiquidityBNB.plus(trackedLiquidityBNB);
  moonwalker.totalLiquidityUSD = moonwalker.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  moonwalker.totalTransactions = moonwalker.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let moonwalker = MoonwalkerFactory.load(FACTORY_ADDRESS);
  moonwalker.totalVolumeUSD = moonwalker.totalVolumeUSD.plus(trackedAmountUSD);
  moonwalker.totalVolumeBNB = moonwalker.totalVolumeBNB.plus(trackedAmountBNB);
  moonwalker.untrackedVolumeUSD = moonwalker.untrackedVolumeUSD.plus(derivedAmountUSD);
  moonwalker.totalTransactions = moonwalker.totalTransactions.plus(ONE_BI);
  ...
  let moonwalkerDayData = updateMoonwalkerDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  moonwalkerDayData.dailyVolumeUSD = moonwalkerDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  moonwalkerDayData.dailyVolumeBNB = moonwalkerDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  moonwalkerDayData.dailyVolumeUntracked = moonwalkerDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  mooonwalkerDayData.save();
